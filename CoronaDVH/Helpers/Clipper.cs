...
//use_xyz: adds a Z member to Vector2l. Adds a minor cost to performance.
//#define use_xyz
...
namespace ClipperLib
{
    ...
    using Path = List<Vector2l>;
    using Paths = List<List<Vector2l>>;
    ...
    public struct DoublePoint
    {
        ...
        public DoublePoint(Vector2l ip)
        {
    ...
    {
        internal Vector2l Bot;
        internal Vector2l Curr; //current (updated for every new scanbeam)
        internal Vector2l Top;
        internal Vector2l Delta;
        internal double Dx;
...
#if use_int32
    public const long loRange = 0x7FFF;
    public const long hiRange = 0x7FFF;
#else
        ...
        internal bool PointIsVertex(Vector2l pt, OutPt pp)
        {
            OutPt pp2 = pp;
            do
            {
        ...
        internal bool PointOnLineSegment(Vector2l pt,
            Vector2l linePt1, Vector2l linePt2, bool UseFullRange)
        {
            if (UseFullRange)
        ...
        internal bool PointOnPolygon(Vector2l pt, OutPt pp, bool UseFullRange)
        {
            OutPt pp2 = pp;
            while (true)
            {
        ...
        internal static bool SlopesEqual(Vector2l pt1, Vector2l pt2,
            Vector2l pt3, bool UseFullRange)
        {
            if (UseFullRange)
        ...
        internal static bool SlopesEqual(Vector2l pt1, Vector2l pt2,
            Vector2l pt3, Vector2l pt4, bool UseFullRange)
        {
            if (UseFullRange)
        ...
        void RangeTest(Vector2l Pt, ref bool useFullRange)
        {
            if (useFullRange)
            {
      ...
      if (!Closed)
        ...
        internal bool Pt2IsBetweenPt1AndPt3(Vector2l pt1, Vector2l pt2, Vector2l pt3)
        {
    ...
    public class Clipper : ClipperBase
    {
...
#if use_xyz
      public delegate void ZFillCallback(Vector2l bot1, Vector2l top1, 
        Vector2l bot2, Vector2l top2, ref Vector2l pt);
      public ZFillCallback ZFillFunction { get; set; }
        ...
        private void AddJoin(OutPt Op1, OutPt Op2, Vector2l OffPt)
        {
            Join j = new Join();
            j.OutPt1 = Op1;
            j.OutPt2 = Op2;
        ...
        private void AddGhostJoin(OutPt Op, Vector2l OffPt)
        {
            Join j = new Join();
            j.OutPt1 = Op;
            j.OffPt = OffPt;
...
#if use_xyz
      internal void SetZ(ref Vector2l pt, TEdge e1, TEdge e2)
      {
      ...
      //------------------------------------------------------------------------------
        ...
        private void AddLocalMaxPoly(TEdge e1, TEdge e2, Vector2l pt)
        {
            AddOutPt(e1, pt);
        ...
        private OutPt AddLocalMinPoly(TEdge e1, TEdge e2, Vector2l pt)
        {
            OutPt result;
            TEdge e, prevE;
            ...
            if (prevE != null && prevE.OutIdx >= 0 && prevE.Top.Y < pt.Y && e.Top.Y < pt.Y)
            {
                ...
                if ((xPrev == xE) && (e.WindDelta != 0) && (prevE.WindDelta != 0) &&
                  SlopesEqual(new Vector2l(xPrev, pt.Y), prevE.Top, new Vector2l(xE, pt.Y), e.Top, m_UseFullRange))
                {
        ...
        private OutPt AddOutPt(TEdge e, Vector2l pt)
        {
            if (e.OutIdx < 0)
            {
        ...
        internal void SwapPoints(ref Vector2l pt1, ref Vector2l pt2)
        {
            Vector2l tmp = new Vector2l(pt1);
            pt1 = pt2;
            pt2 = tmp;
        }
        //------------------------------------------------------------------------------
        ...
        private double GetDx(Vector2l pt1, Vector2l pt2)
        {
        ...
        private void IntersectEdges(TEdge e1, TEdge e2, Vector2l pt)
        {
            ...
            for (; ; ) //loop through consec. horizontal edges
            {
                ...
                while (e != null)
                {
                        ...
                        if (dir == Direction.dLeftToRight)
                        {
                            while (currMax != null && currMax.X < e.Curr.X)
                            {
                                if (horzEdge.OutIdx >= 0 && !IsOpen)
                                    AddOutPt(horzEdge, new Vector2l(currMax.X, horzEdge.Bot.Y));
                                currMax = currMax.Next;
                            }
                        }
                        else
                  ...
                  if (dir == Direction.dLeftToRight) SetZ(ref e.Curr, horzEdge, e);
                    ...
                    if (dir == Direction.dLeftToRight)
                    {
                        Vector2l Pt = new Vector2l(e.Curr.X, horzEdge.Curr.Y);
            ...
            while (isModified && m_SortedEdges != null)
            {
                ...
                while (e.NextInSEL != null)
                {
                    ...
                    if (e.Curr.X > eNext.Curr.X)
                    {
                        ...
                        if (pt.Y < topY)
                            pt = new Vector2l(TopX(e, topY), topY);
        ...
        private void IntersectPoint(TEdge edge1, TEdge edge2, out Vector2l ip)
        {
            ip = new Vector2l();
            double b1, b2;
              ...
              if (e.Top.Y == topY) e.Curr.Z = e.Top.Z;
                    ...
                    if (StrictlySimple)
                    {
                        ...
                        if ((e.OutIdx >= 0) && (e.WindDelta != 0) && ePrev != null &&
                          ...
                          (ePrev.WindDelta != 0))
                        {
                            Vector2l ip = new Vector2l(e.Curr);
#if use_xyz
                SetZ(ref ip, ePrev, e);
#endif
        ...
        public static int PointInPolygon(Vector2l pt, Path path)
        {
            ...
            for (int i = 1; i <= cnt; ++i)
            {
                Vector2l ipNext = (i == cnt ? path[0] : path[i]);
                if (ipNext.Y == pt.Y)
                {
        ...
        //See "The Point in Polygon Problem for Arbitrary Polygons" by Hormann & Agathos
        //http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.88.5498&rep=rep1&type=pdf
        private static int PointInPolygon(Vector2l pt, OutPt op)
        {
            //returns 0 if false, +1 if true, -1 if pt ON polygon boundary
            int result = 0;
            OutPt startOp = op;
        ...
        private static double DistanceSqrd(Vector2l pt1, Vector2l pt2)
        {
        ...
        private static double DistanceFromLineSqrd(Vector2l pt, Vector2l ln1, Vector2l ln2)
        {
        ...
        private static bool SlopesNearCollinear(Vector2l pt1,
            Vector2l pt2, Vector2l pt3, double distSqrd)
        {
        ...
        private static bool PointsAreClose(Vector2l pt1, Vector2l pt2, double distSqrd)
        {
            double dx = (double)pt1.X - pt2.X;
            double dy = (double)pt1.Y - pt2.Y;
        ...
        internal static Paths Minkowski(Path pattern, Path path, bool IsSum, bool IsClosed)
        {
            ...
            if (IsSum)
                for (int i = 0; i < pathCnt; i++)
                {
                    ...
                    foreach (Vector2l ip in pattern)
                        p.Add(new Vector2l(path[i].X + ip.X, path[i].Y + ip.Y));
                    result.Add(p);
                }
            else
                for (int i = 0; i < pathCnt; i++)
        ...
        private static Path TranslatePath(Path path, Vector2l delta)
        {
            Path outPath = new Path(path.Count);
            for (int i = 0; i < path.Count; i++)
                outPath.Add(new Vector2l(path[i].X + delta.X, path[i].Y + delta.Y));
            return outPath;
        }
        //------------------------------------------------------------------------------
    ...
    public class ClipperOffset
    {
        ...
        public void AddPath(Path path, JoinType joinType, EndType endType)
        {
            ...
            if (m_lowest.X < 0)
                m_lowest = new Vector2l(m_polyNodes.ChildCount - 1, k);
            else
            {
                Vector2l ip = m_polyNodes.Childs[(int)m_lowest.X].m_polygon[(int)m_lowest.Y];
        ...
        internal static DoublePoint GetUnitNormal(Vector2l pt1, Vector2l pt2)
        {
        ...
        private void DoOffset(double delta)
        {
            ...
            for (int i = 0; i < m_polyNodes.ChildCount; i++)
            {
                ...
                else
                {
                    ...
                    {
                        int j = len - 1;
                        pt1 = new Vector2l((long)Round(m_srcPoly[j].X + m_normals[j].X *
                    ...
                    if (node.m_endtype == EndType.etOpenButt)
                    {
                        pt1 = new Vector2l((long)Round(m_srcPoly[0].X - m_normals[0].X * delta),
        ...
        public void Execute(ref Paths solution, double delta)
        {
            ...
            else
            {
                ...
                outer.Add(new Vector2l(r.left - 10, r.bottom + 10));
                outer.Add(new Vector2l(r.right + 10, r.bottom + 10));
                outer.Add(new Vector2l(r.right + 10, r.top - 10));
                outer.Add(new Vector2l(r.left - 10, r.top - 10));
        ...
        void OffsetPoint(int j, ref int k, JoinType jointype)
        {
            ...
            if (Math.Abs(m_sinA * m_delta) < 1.0)
            {
                ...
                {
                    m_destPoly.Add(new Vector2l(Round(m_srcPoly[j].X + m_normals[k].X * m_delta),
            ...
            if (m_sinA * m_delta < 0)
            {
                ...
                m_destPoly.Add(new Vector2l(Round(m_srcPoly[j].X + m_normals[j].X * m_delta),
        ...
        {
            double q = m_delta / r;
            m_destPoly.Add(new Vector2l(Round(m_srcPoly[j].X + (m_normals[k].X + m_normals[j].X) * q),
